---
tipo: aula
domínio: marketing/tráfego-pago
tema: Tráfego + Prospecção
objetivo: Encher funil com leads qualificados
método: Venda Sem Esforço
duração: 90min
criado: 2025-01-12
...
```mermaid
graph LR
    A[Lead entra] --> B{Já sabe}
    B --> C[O que você faz]
    B --> D[Como você faz]
    B --> E[Por que você é diferente]
    C --> F[Lead Pré-Vendido]
    D --> F
    E --> F
    F --> G[Venda Sem Esforço]
```
...
```mermaid
graph TD
    A[Soluções Comuns] --> B[Efeito 1: CPL Alto<br/>Qualidade Baixa]
    A --> C[Efeito 2: 90% Somem<br/>Após Material Grátis]
    A --> D[Efeito 3: Reuniões<br/>com Curiosos]
    A --> E[Efeito 4: Direct cheio<br/>de 'Quanto custa?']
    ...
    F --> G[Funil Furado<br/>R$ Perdido<br/>Tempo Desperdiçado]
...
```mermaid
graph LR
    subgraph "Lead Comum (Solução Comum)"
        A1[Clicou em 'Saiba Mais'] --> A2[Não sabe quem você é]
        A2 --> A3[Pergunta: Quanto custa?]
        A3 --> A4[Você precisa apresentar TUDO]
        A4 --> A5[Objeções em série]
        A5 --> A6[Follow-up infinito]
        A6 --> A7[80% não compra]
    end

    subgraph "Lead Qualificado (Nossa Estratégia)"
        B1[Consumiu Post N3] --> B2[Já conhece seu Método]
        B2 --> B3[Pergunta: Como funciona pra mim?]
        B3 --> B4[Você só personaliza]
        B4 --> B5[Objeções já foram tratadas]
        B5 --> B6[Decisão rápida]
        B6 --> B7[60-70% compra]
    end
...
```mermaid
graph TD
    A[Funil Qualificado] --> B[Pilar 1:<br/>CONTEÚDO ESTRATÉGICO]
    A --> C[Pilar 2:<br/>TRÁFEGO INTELIGENTE]
    A --> D[Pilar 3:<br/>ATENDIMENTO<br/>AUTOMATIZADO +<br/>PROSPECÇÃO SELETIVA]

    B --> B1[Posts N3 como base]
    B --> B2[Stories de remarketing]
    B --> B3[Destaques = autoridade]

    C --> C1[Meta Ads distribuindo<br/>conteúdo, não ofertas]
    C --> C2[Segmentação por<br/>consciência]
    C --> C3[R$30/dia = ROI positivo]

    D --> D1[Script Automático<br/>no Direct]
    D --> D2[Prospecção só em<br/>quem já interagiu]
    D --> D3[Combinação inbound<br/>+ outbound]

    style A fill:#4c6ef5,stroke:#364fc7,stroke-width:4px,color:#fff
...
```mermaid
sequenceDiagram
    participant A as Anúncio Genérico
    participant L as Lead
    participant P as Perfil Vazio
    participant D as Dúvida

    A->>L: "Clique aqui!"
    L->>P: Visita perfil
    P->>L: Bio genérica, posts aleatórios
    L->>D: "Quem é essa pessoa?"
    D->>L: Sai e nunca volta
```

**Fluxo Correto (Nossa Estratégia):**
```mermaid
sequenceDiagram
    participant A as Anúncio de Post N3
    participant L as Lead
    participant P as Perfil Estratégico
    participant C as Convicção
    participant D as Direct

    A->>L: Apresenta Método + Contraste
    L->>P: Visita perfil
    P->>L: 3 Posts Fixados + Destaques + Stories
    L->>C: "Isso faz sentido!"
    C->>D: Envia palavra-chave
```
...
```mermaid
graph TD
    A[Público Total] --> B[Nível 1:<br/>Inconsciente<br/>Não sabe que tem problema]
    A --> C[Nível 2:<br/>Consciente do Problema<br/>Sabe que tem problema]
    A --> D[Nível 3:<br/>Consciente da Solução<br/>Sabe que existe solução]
    A --> E[Nível 4:<br/>Consciente do Produto<br/>Conhece soluções específicas]
    A --> F[Nível 5:<br/>Consciente da Compra<br/>Pronto para comprar]

    B --> G[NÃO anunciar aqui<br/>Custo alto, conversão baixa]
    C --> H[NÃO anunciar aqui<br/>Precisa educação longa]
    D --> I[FOCO AQUI<br/>Post N3 de Contraste]
    E --> J[FOCO AQUI<br/>Remarketing]
    F --> K[FOCO AQUI<br/>Oferta direta]
...
```mermaid
graph TD
    A[Lead Qualificado<br/>Entra no Sistema] --> B{De onde veio?}

    B -->|Canal 1| C[ANÚNCIO<br/>Inbound]
    B -->|Canal 2| D[PROSPECÇÃO<br/>Outbound]
    B -->|Canal 3| E[ORGÂNICO<br/>Stories]

    C --> F[Responde palavra-chave<br/>no Direct]
    D --> G[Conversa manual<br/>personalizada]
    E --> H[Responde palavra-chave<br/>via Stories]

    F --> I[Script Automático<br/>ManyChat]
    G --> J[Qualificação Manual]
    H --> I

    I --> K{Lead Qualificado?}
    J --> K

    K -->|Sim| L[Proposta Principal<br/>ou Reunião]
    K -->|Não| M[Downsell ou<br/>Conteúdo Gratuito]
...
```mermaid
journey
    title Jornada do Lead: Da Descoberta à Conversão
    section Descoberta (Anúncio)
      Vê Post N3 no feed: 5: Lead
      Identifica com Problema Sofisticado: 5: Lead
      Lê sobre o Método: 4: Lead
      Vê Contraste com Soluções Comuns: 5: Lead
    section Interesse (Perfil)
      Clica no perfil: 5: Lead
      Vê Post 1/3 (Explicativo): 4: Lead
      Vê Post 2/3 (Transformação): 5: Lead
      Vê Post 3/3 (Contraste): 5: Lead
      Confere Destaques e Provas Sociais: 4: Lead
    section Engajamento (Interação)
      Decide interagir: 5: Lead
      Envia palavra-chave no Direct: 5: Lead
    section Qualificação (Perguntas)
      Responde: O que vende?: 4: Lead
      Responde: Estratégia atual?: 4: Lead
      Responde: Capacidade de investimento?: 3: Lead
      Responde: Faturamento atual e objetivo?: 4: Lead
    section Conversão (Decisão)
      Lead qualificado recebe proposta: 5: Lead, Sistema
      Toma decisão de compra: 5: Lead
```
...
```mermaid
graph LR
    A[Lead clica<br/>no perfil] --> B[BIO]
    A --> C[POSTS FIXADOS]
    A --> D[DESTAQUES]
    A --> E[STORIES RECENTES]

    B --> B1["João Silva<br/>📍 Venda Sem Esforço<br/>Ajudo prestadores de serviço<br/>a venderem R$30k+/mês<br/>sem reuniões intermináveis<br/><br/>👇 Comece aqui: POST N3"]

    C --> C1[Post 1: O que é o Método]
    C --> C2[Post 2: Como fica sua vida]
    C --> C3[Post 3: Por que é melhor]

    D --> D1[Comece Aqui]
    D --> D2[Resultados]
    D --> D3[Depoimentos]

    E --> E1[Stories do dia<br/>mostrando rotina<br/>ou case]
...
```mermaid
graph TD
    A[Lead decidiu<br/>interagir] --> B{Qual ação?}

    B -->|Opção A| C[Responde palavra-chave<br/>no Direct]
    B -->|Opção B| D[Segue mas não<br/>interage ainda]
    B -->|Opção C| E[Salva post +<br/>Alta intenção]

    C --> F[Script Automático<br/>ManyChat]
    D --> G[Remarketing Orgânico<br/>via Stories]
    E --> H[Prospecção Ativa<br/>Personalizada]

    F --> I{Lead responde<br/>perguntas?}
    G --> J[Aguarda interação<br/>nos próximos 7 dias]
    H --> K[Conversa manual<br/>1:1]

    I -->|Sim| L[Segue para qualificação]
    I -->|Não| M[Remove da automação]
    J -->|Interagiu| L
    J -->|Não interagiu| N[Remove do remarketing]
    K --> L
...
sequenceDiagram
    participant L as Lead
    participant B as Bot (ManyChat)
    participant S as Sistema

    L->>B: Envia "POST N3"
    B->>L: Oi [Nome], antes de tudo,<br/>qual seu primeiro nome?
    L->>B: João
    B->>L: João, o que você viu pode<br/>adicionar R$30k/mês ao seu negócio.<br/><br/>Interesse em continuar?
    L->>B: Sim
    B->>L: Ótimo! Para te mostrar como,<br/>preciso que responda 4 perguntas.<br/><br/>1. O que você vende/oferece?
    L->>B: Consultoria de marketing
    B->>L: 2. Atualmente, o que você faz<br/>para conseguir clientes?
    L->>B: Faço posts e alguns anúncios
    B->>L: 3. Está disposto a investir<br/>pelo menos R$30/dia em tráfego?
    L->>B: Sim, sem problemas
    Note over S: ✅ LEAD QUALIFICADO<br/>Passou no filtro de budget
    B->>L: 4. Qual seu faturamento atual<br/>e objetivo nos próximos 60 dias?
    L->>B: Fatura R$15k, quer R$30k
    B->>S: Salva dados + Tag "Qualificado"
    B->>L: Perfeito, João! Vou analisar<br/>e retorno em até 24h.<br/><br/>Digite "OK" para confirmar.
    L->>B: OK
    Note over S: Notifica atendente<br/>Lead qualificado aguardando
```
...
```mermaid
graph TD
    A[Pergunta 3] --> B{Resposta?}

    B -->|Sim| C[✅ Lead Qualificado<br/>Tem budget<br/>Entende investimento]
    B -->|Não| D[❌ Lead Desqualificado<br/>Sem budget<br/>Quer gratuito]

    C --> E[Segue no fluxo<br/>Recebe proposta]
    D --> F[Oferece Downsell<br/>ou conteúdo gratuito]

    C --> G[70% de conversão<br/>para proposta]
    D --> H[5% de conversão<br/>para downsell]
...
```mermaid
graph TD
    subgraph "❌ Estratégia Comum (ERRADA)"
        A1[Objetivo: Conversão<br/>Compre agora!]
        A2[Lead Magnet: eBook grátis]
        A3[Segmentação ampla<br/>interesse: marketing]
        A4[Budget alto<br/>R$100-300/dia]
        A5[Resultado:<br/>CPL R$10-30<br/>Conversão 2-5%<br/>ROI negativo]
    end

    subgraph "✅ Nossa Estratégia (CERTA)"
        B1[Objetivo: Engajamento<br/>Leia o post]
        B2[Conteúdo: Post N3<br/>valor real]
        B3[Segmentação por<br/>consciência nível 3-4]
        B4[Budget baixo<br/>R$30/dia]
        B5[Resultado:<br/>CPL R$0,50-2<br/>Conversão 15-25%<br/>ROI positivo]
    end
...
```mermaid
graph TD
    A[CAMPANHA 1<br/>Topo de Funil] --> B[Objetivo]
    A --> C[Budget]
    A --> D[Criativo]
    A --> E[Segmentação]
    A --> F[Resultado Esperado]

    B --> B1[Engajamento /<br/>Tráfego para perfil]
    C --> C1[R$30/dia<br/>R$900/mês]
    D --> D1[Post N3 3/3<br/>Contraste]
    E --> E1[25-55 anos<br/>Interesses: concorrentes<br/>Comportamento: empreendedor]
    F --> F1[200-400 cliques<br/>no perfil/mês<br/>60-100 seguidores<br/>30-50 DMs]

    style A fill:#4c6ef5,color:#fff,stroke:#364fc7,stroke-width:3px
```
...
```mermaid
graph TD
    A[CAMPANHA 2<br/>Meio de Funil] --> B[Público Personalizado]
    A --> C[Budget]
    A --> D[Criativo]
    A --> E[Objetivo]

    B --> B1[Visitaram perfil 14d]
    B --> B2[Engajaram posts<br/>salvaram, comentaram]
    B --> B3[Viram 50%+ Stories]

    C --> C1[R$20/dia<br/>R$600/mês]

    D --> D1[Sequência Stories<br/>Remarketing Orgânico]

    E --> E1[Mensagens<br/>Direct]

    style A fill:#ffd43b,stroke:#f59f00,stroke-width:3px
```
...
```mermaid
graph TD
    A[CAMPANHA 3<br/>Fundo de Funil] --> B[Público]
    A --> C[Budget]
    A --> D[Criativo]
    A --> E[Objetivo]

    B --> B1[Interagiram Direct<br/>mas não compraram]
    B --> B2[Seguem há 7+ dias<br/>consomem conteúdo]

    C --> C1[R$15/dia<br/>R$450/mês]

    D --> D1[Vídeo/Reels curto<br/>Case de sucesso]

    E --> E1[Conversões<br/>Enviar palavra-chave VIP]
...
```mermaid
graph LR
    A[Estágio do Lead] --> B{Qual criativo?}

    B -->|Não conhece você| C[TIPO 1:<br/>Post N3 Carrossel<br/>Contraste]
    B -->|Já visitou perfil| D[TIPO 2:<br/>Stories Estáticos<br/>Remarketing]
    B -->|Seguidor 7+ dias| E[TIPO 3:<br/>Vídeo/Reels<br/>Case de Sucesso]

    C --> C1[Campanha 1<br/>Topo de Funil]
    D --> D1[Campanha 2<br/>Meio de Funil]
    E --> E1[Campanha 3<br/>Fundo de Funil]
